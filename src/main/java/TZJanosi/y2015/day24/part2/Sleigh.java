...
public class Sleigh {
    ...
    private int getReachableIncrement(int shortest) {
        Container footContainer = getSlots().get(0);
        int numberOfRemainingPackages = shortest - footContainer.getNumberOfPackages();
        if (numberOfRemainingPackages >= maxReachableIncrement.size()) {
            return weightLimit;
        } else {
            return maxReachableIncrement.get(numberOfRemainingPackages);
        }
    }
