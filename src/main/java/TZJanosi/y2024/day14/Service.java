...
public class Service {
    ...
    private int createRegionFrom(Coordinate point) {
        ...
        if(alreadyCheckedCoordinates.contains(point)){
            ...
            for (Coordinate neighbour:neighbours) {
                if(! alreadyCheckedCoordinates.contains(neighbour)){
                    if(ExistRobotOnThisCoordinate(neighbour)){
    ...
    public void findNumberOfRobotsInQuadrants(){
        numberOfRobotsInQuadrants=robots.stream().collect(Collectors.groupingBy(r->r.getQuadrant(),Collectors.counting()));
    }
