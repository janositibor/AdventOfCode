...
public class SecretNumberGenerator {
    ...
    private static int DIVIDER = 32;
    private static int SECOND_MULTIPLIER = 2048;
    private static long VALUE_FOR_MODULO_IN_PRUNE = 16777216L;
    private static int NUMBER_OF_CHECKED_CHANGES = 4;
    private int numberOfIterations;
    private long value;
    private List<Integer> prices = new ArrayList<>();
    private List<Integer> changes = new ArrayList<>();
    private Map<List<Integer>, Integer> pricesToChangePatterns = new HashMap<>();
    ...
    public void createPricesToChangePatterns(int indexInPrice, Integer price, int maxLengthOfChangePattern) {
        ...
        for (int i = 0; i < maxLengthOfPattern; i++) {
            if (actualKey.size() == NUMBER_OF_CHECKED_CHANGES) {
                if (!pricesToChangePatterns.containsKey(actualKey)) {
