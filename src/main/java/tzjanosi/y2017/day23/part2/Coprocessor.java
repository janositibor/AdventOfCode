...
public class Coprocessor {
    ...
    public int executePart2() {
        int registerB = (int) findValueAfterPartialRun("b", 8);
        int registerC = (int) findValueAfterPartialRun("c", 8);
        int step = (int) (-1 * operations.get(operations.size() - 2).getParameter2Value());

        int output = 0;
        for (int i = registerB; i <= registerC; i += step) {
            if (!prime(i)) {
                output++;
            }
        }
    ...
    private long findValueAfterPartialRun(String nameOfregister, int limit) {
        execute(limit);
        return findOrCreateRegisterByName(nameOfregister).getValue();
    }

    private boolean prime(int numberToCheck) {
    ...
    private Register findOrCreateRegisterByName(String registerName) {
        ...
        Register output;
        if ("one".equals(registerName) || "a".equals(registerName)) {
