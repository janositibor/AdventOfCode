...
public class Disk {
    ...
    public long calculateCheckSum(){
        return area.stream().mapToLong(Fragment::checkSum).reduce(0, (n, m) -> n + m);
    }
...
//                    System.out.println(to.getOrder() + " <? " + from.getOrder() + "(" +from.getFileID()+")");
    ...
    private int getMaxFileId() {
        return area.stream()
                ...
                .sorted(Comparator.comparingInt(Fragment::getOrder).reversed())
                .findFirst()
                .get()
                .getFileID();
    }
    ...
    private Optional<Fragment> findFirstLongEnoughBlank(int length) {
        return area.stream()
                ...
                .sorted(Comparator.comparingInt(Fragment::getOrder))
                .findFirst();
    }
