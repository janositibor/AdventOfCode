...
public class StoneService {
    ...
    private Map<Long, Map<Integer, Long>> memory = new ConcurrentHashMap<>();
    ...
    private long leafCounter(String value) {
        ...
        String valueToCheck = length > 1 ? removeLeadingZeros(value) : value;
        length = valueToCheck.length();
        if(length%2==0){
            return 2L;
        }
        return 1L;
    ...
    private String[] next(String value) {
        ...
        length = valueToCheck.length();
        if ("0".equals(valueToCheck)) {
            result= new String[] {"1"};
            return result;
        }
        if(length%2==0){
            int halfLength=length/2;
            result = new String[]{valueToCheck.substring(0, halfLength), valueToCheck.substring(halfLength)};
            return result;
        }
        result = new String[]{Long.toString(Long.parseLong(valueToCheck) * 2024)};
        return result;
    }
