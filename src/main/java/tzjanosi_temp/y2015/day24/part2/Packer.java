package tzjanosi_temp.y2015.day24.part2;
...
public class Packer {
    ...
    private long getQeFromFootSlots() {
        return footSlots.stream().mapToLong(Container::getQe).min().getAsLong();
    }

    private void fillFootSlots() {
        int i = minNumberOfPackagesInFootSlots;
        while (footSlots.isEmpty()) {
    ...
    private void replaceNextElement(Sleigh sleigh) {
        Sleigh child;
        for (int i = 0; i < sleigh.getSlots().size() && !found; i++) {
            if (sleigh.isValid()) {
